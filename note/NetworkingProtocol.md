...
- 停止等待: 就是每发完一个分组就停止发送，等待对方确认，在收到确认后在发送下一个分组; 发送放在一定时间后没有收到确认会重新发送

- 超时重传: 
	- TCP每发送一个分组就会对这个分组设置一次计时器。只要计时器设置的重传时间到期但还没有收到确认，就要重传这一分组
	- 当传输出现差错的时候, 接收方会丢弃出错的分组，但并不会通知发送方出错的情况，而是等待计时器到期让发送方重传

![](../static/images/network_tcp007.webp)

- 确认丢失: 当发送方在计时器规定时间内没有收到接收方的确认ACK，就重发分组

- 确认迟到: 当接收方又收到了重传的分组(M1)，这时将丢弃重复的分组(M1),接收方并再次发送确认ACK(因为发送方重传就表示没有收到确认)

![](../static/images/network_tcp008.webp)
...
![](../static/images/network_tcp010.jpg)

- 累计确认: 接收方不必对收到的分组组个发送确认，而是对按序到达的最后一个分组发送确认; 但是有缺陷: 例如发送方传送了5个包，但是第3个包丢失了，第4和第5个正常。这时根据累积确认，接收方只能传送前两个包的ACK（按序到达的最后一个）。在接收到第2个包ACK的发送方不知道后面3个包是否到达，就会将后3个包全部进行重传。即：Go-back-N
	- 优点: 容易实现，即使部分确认丢失也不必重传
	- 缺点: 不能向发送方反映出接收方已经正确收到的所有分组的信息

##### 滑动窗口协议

> 滑动窗口协议是指数据发送方有一个发送窗口，发送窗口范围内的数据允许发送，随着接收方传来的确认信息以及通知的接收窗口的大小来动态调整发送窗口的起始位置以及大小。

- 该协议适用于对按顺序传送分组的可靠性要求较高的环境。比如`TCP`中
	- 提供TCP的可靠性（最基本的传输可靠性来源于“确认重传”机制），避免丢包
	- 提供TCP的流控特性，用于网络数据传输时的流量控制，以避免拥塞的发生

和停止等待协议的对比图：

![](../static/images/network_tcp009.webp)
...
![](../static/images/network_tcp011.png)

示例图说明：
1. 发送方A和接收方B建立TCP连接时，B告诉A接收窗口大小为400字节(按图中的示例)
2. 假设1格代表100字节; t1时, 发送应用程序向缓存写入800字节(8个分组); 由于发送窗口为400字节，A向B按顺序发送第1~4分组
3. 在t2时, B收到四个分组放入接收缓存中的接收窗口，并按TCP首部的需要排序分组，当窗口中的分组编号连续时，接收窗口向前移动。应用程序按顺序读取接收窗口外连续的分组数据
...
SACK(选择确认): 只重传丢失或出错的分组; “确认”存在的问题：如果发送序列中间的某个数据包丢失，TCP会重传最后确认的分组的后续分组，这样原先己经正确传输的分组也可能重复发送，降低了TCP性能; 比如`滑动窗口协议`中的例子, 窗口中7的分组丢失导致8的分组也被重传; 因此`选择确认`要求接收方告诉发送发哪些数据丢弃或出错、哪些收到;
...
应用程序把数据传送到缓存后，剩下的发送任务就由TCP来控制了

TCP用不同的机制来控制报文段的发送时机，机制包括：
- MSS: TCP维持一个持续变量，他等于最大报文段长度MSS（类似以太网协议中的MTU值）。只要缓存中存放的数据达到MSS字节时，就组成一个TCP报文段发送出去。当数据长度超过MSS值，就会根据MSS值分多次进行传输，叫做分段。
- Nagle(纳格)算法
...
> 注： 滑动窗口是接收端的流控; nagle是为了尽可能发送大块数据,避免网络中充斥着许多小数据块,从而提升传输效率;

#### 粘包现象

##### 概述

TCP 传输协议是面向流的，没有数据包界限，也就是说消息无边界。客户端向服务端发送数据时，可能将一个完整的报文拆分成多个小报文进行发送，也可能将多个报文合并成一个大的报文进行发送。（TCP协议的底层，并不了解上层业务的具体定义，它会根据TCP缓冲区的实际情况进行包的划分。在业务层面认为一个完整的包，可能会被TCP拆分成多个小包进行发送，也可能把多个小的包封装成一个大的数据包进行发送，这就是所谓的TCP粘包拆包问题。）。因此就有了拆包和粘包。 在网络通信的过程中，每次可以发送的数据包大小是受多种因素限制的，如 MTU 传输单元大小、滑动窗口等。
...
![](../static/images/network_tcp012.png)

根据上图示例说明，客户端发送了两个数据包P1和P2给服务端，服务端一次读取到的字节数是不确定的，可能存在以下4种情况:
1. 服务端分两次读取到了两个独立的数据包P1和P2，没有粘包和拆包；
...
上面是客户端数据处理的情况，服务器数据处理的情况也会发生粘包: 服务器收到的数据会保存在缓存中，如果应用层提取数据不够快就会导致缓存中多条数据粘在一起

##### 解决方法

不是什么时候都要去处理粘包，当TCP是长连接，并且传输的是结构化数据时，如：传送的是一个结构体类型的数据，由于不知道结构化数据的边界，容易导致粘包问题的出现。这时需要考虑粘包问题的影响；

当TCP是短连接，即只进行一次数据通信过程，通信完成就关闭连接，这样就不会出现粘包问题。如果传输的是字符串、文件等无结构化数据时，也不会出现粘包问题。因为发送方只管发送，接收方只管接收存储就行了
...
> 拥塞控制是作用于网络的，防止过多的数据注入到网络中, 避免网络中的路由器或者链路过载
...
拥塞窗口 cwnd（congestion window）是发送方维护的一个的状态变量，拥塞窗口的大小取决于网络的拥塞程度，并且在动态的发生变化，发送方让自己的发送窗口等于拥塞窗口 (注：在这里我们假设接受方总是有足够大的缓存空间，因而发送窗口的大小取决于网络的拥塞程度，`实际上真正的窗口值 = min(接收方窗口，拥塞窗口)`

发送方控制拥塞窗口cwnd的原则是：只要网络没有出现拥塞，拥塞窗口就可以在增大一些，以便把更多的分组发送出去。但是只要网络出现拥塞或有可能出现拥塞，就必须把拥塞窗口减小一些，以减少注入到网络中的分组数，以便环节网络出现的拥塞。
...
![](../static/images/network_tcp013.webp)

##### 拥塞避免

为了防止拥塞窗口cwnd增长过大引起网络拥塞，还需要设置一个慢开始门限ssthresh状态变量

- ssthresh(slow start threshold)：慢开始阈值，cwnd达到阈值后，开始拥塞避免（加法增大）;当发生网络拥塞时那么执行ssthresh乘法减小
- 加法增大: 拥塞窗口cwnd缓慢增大(把发送方的拥塞窗口+1，而不是像慢开始那样成倍的增加)，以防止网络过早出现拥塞
- 乘法减小: 只要出现网络拥塞，把ssthresh减为拥塞峰值的一半，同时执行慢开始算法（cwnd又恢复到初始值）

![](../static/images/network_tcp014.webp)

图解：
- 开始时执行慢开始，cwnd指数增长
- 当cwnd达到ssthresh后开始执行拥塞避免即加法增大
- 当加法增大到一定程度出现网络拥塞后执行乘法减小，并重新执行慢开始(cwnd恢复到初始值)
...
图示: 
![](../static/images/network_tcp015.webp)

图解: 
接收方收到了M1和M2后都分别及时发出了确认。现假定接收方没有收到M3但却收到了M4。本来接收方可以什么都不做。但按照快重传算法，接收方必须立即发送对M2的重复确认，以便让发送方尽早知道接收方没有收到报文段M3。发送方接着发送M5和M6.接收方收到后也仍要再次分别发出对M2的重复确认。
...
当发送方收到三个重复确认(3-ACK)时不在认为是网络拥塞而是丢失了个别报文段，于是不启动慢开始(慢开始cwnd恢复到初始值)，而是执行快恢复

快恢复调整规则
1. 调整门限值`ssthresh = cwnd / 2`
2. 把cwnd值设置为新的ssthresh值

下中④将不会执行慢开始，而是快恢复

![](../static/images/network_tcp016.webp)
...
http(Hyper Text Transfer Protocol)协议是超文本传输协议，是基于请求与响应的，无状态，无连接的协议，常基于TCP／IP协议传输数据

#### 特点

- http是无状态：http协议是无状态协议。无状态协议是指：协议对客户端没有状态存储，即对事物的处理没有“记忆”能力，比如访问一个网站需要重复的登录操作；解决方法：	
	- 通过cookies/session会话保存;
	- 持久连接（HTTP keep-alive）方法，只要任意一端没有明确提出断开连接
- http是无连接：无连接的含义是指限制每次只处理一个请求，服务器处理完客户的请求，并接收到客户的应答后，服务器就会断开宇客户端的连接。采用这种方式可以节省传输时间。
- http是基于请求和响应：基本特性就是客户端发送请求，服务端响应。
- 通信使用明文，请求和响应不会对通信方式进行确认，无法保证数据的完整性。
- 简单快速、灵活。

常见的请求方法: GET,POST,PUT,DELETE,HEAD,PATCH等等

#### 报文格式

![](../static/images/network_http001.webp)
...
- 405 Method Not Allowed：服务器禁止了使用当前HTTP方法的请求（请求方式错误）
...
HTTPS (HyperText Transfer Protocol Secure)，译为：超文本传输安全协议; 常称为`HTTP over TLS、HTTP over SSL、HTTP Secure`, 由网景公司1994年首次提出; 默认端口`443`;

HTTP协议默认是采取明文传输的，因此会有很大的安全隐患，常见的提高安全性的方法是对通信内容进行加密后再进行传输
...
- TLS(Transport Layer Security)协议，译为：传输层安全性协议; 是SSL协议3.0版本。

常见的加密方式：
- 不可逆-单项散列函数: `MD5、SHA`等
- 可逆-对称加密：`DES、3DES、AES`等，非对称加密：`RSA`等
- 其它-混合密码系统、数字签名、证书
...
- 非对称加密的加解密效率相对于对称加密比较低，而http的应用场景通常是端对端之间的存在大量的交互，非对称加密是无法接受的；
- 在https的场景中只有服务端保存了私钥，一对公私钥只能实现单向的加解密，所以https中内容传输加密方式采用对称加密，而不是非对称加密。

#### TLS连接过程

配合下面的文字看图：

![](../static/images/network_http002.webp)
...
4. Server Key Exchange是ECDHE算法中的一个参数; ECDHE是一个密钥交换算法
5. 告知客户端，协商部分结束; 目前为止客户端和服务端之间已经通过明文共享了，并且客户端也已经拿到服务器的公钥证书，接下来客户端会验证证书的真实有效性
6. Client Key Exchange是用以实现ECDHE算法的另一个参数
7. 发送Change Cipher Spec消息告诉服务器，之后将使用协商好的密钥对数据进行加密在传输
8. 将包含连接至今的全部报文的整体校验值加密之后发送给服务器，这次握手协商是否成功要以服务器是否能正确解密该报文作为判定标准
9. 发送Change Cipher Spec消息告诉客户端，之后将使用协商好的密钥对数据进行加密在传输
10. 给客户端发送加密的报文，能正常返回; 到此为止客户端服务器都验证加解密没问题，握手正式结束; 接下来开始传输加密的HTTP请求和响应
...
网络中进程通信得解决两个问题: 
- 要如何标识一台主机，即怎样确定我们将要通信的进程是在那一台主机上运行?
	> TCP/IP协议族已经帮我们解决了这个问题，网络层的“ip地址”可以唯一标识网络中的主机
- 要如何标识唯一进程; 本地可以通过pid标识，网络中应该怎样标识?
	> 传输层的“协议+端口”可以唯一标识主机中的应用程序（进程），因此，我们利用三元组（ip地址，协议，端口）就可以标识网络的进程了，网络中的进程通信就可以利用这个标志与其它进程进行交互

## 网络IO

IO在计算机中指Input/Output，也就是输入和输出; Unix/Linux系统下IO主要分为磁盘IO，网络IO; 网络IO主要是socket套接字的读(read)、写(write)，socket在Linux系统被抽象为流(stream);

一般的IO就是数据的输入输出，再简单来说，就是从一个地方，到另一个地方, 计算机中，能够存储数据的，一定是存储介质，而且一般采用的存储结构无非是数组、链表、树。而且我们知道，网络中的数据是以二进制的形式传输的，所以，可以简单认为，数据就是二进制数组中的元素。
...
- 阻塞式IO（BIO/blocking IO）
- 非阻塞式IO（NIO/non-blocking IO）
- IO多路复用(IO multiplexing)
- 信号驱动IO
- 异步IO/异步非阻塞IO（AIO/Asynchronous IO）

上面5种除了异步IO其他四种均为同步IO

#### 阻塞/非阻塞/同步/异步

- 阻塞: 阻塞是线程的一个状态，当发起一个请求，调用者一直等待请求结果返回，也就是当前线程会被挂起(线程进入非可执行状态，在这个状态下，cpu不会给线程分配时间片，即线程暂停运行), 无法从事其他任务，只有得到结果才能继续。
- 非阻塞: 非阻塞就是发起一个请求，调用者不用一直等着结果返回，可以先去干其他事情。隔断时间再来询问之前的操作是否完成。这样的过程其实也叫轮询
...
- 阻塞式IO（BIO/blocking IO）
- 非阻塞式IO（NIO/non-blocking IO）net
- IO多路复用(IO multiplexing)
- 信号驱动IO
- 异步IO/异步非阻塞IO（AIO/Asynchronous IO）
...
> [五种网络IO模型](https://blog.csdn.net/qq_42956653/article/details/125613170)

> [一文搞懂网络IO模型（阻塞式IO、非阻塞式IO、IO复用、信号驱动IO、异步IO）](https://www.cnblogs.com/greengages/p/16620971.html)


##### 五种网络IO模型比较

![](../static/images/network_io003.webp)
