...
再看看上图的情况，左图插入11,11比12、14小，比9,10大，因此它应该是需要插入在9、10的位置，但是这个位置不能再插入了，并且双亲结点也已经是3结点了不能继续增加了， 那么继续往上看， 12、14结点的双亲结点8是个2结点；这时我们将9、10拆分，12、14拆分，让根结点8升级为3结点，最终效果看右图
...
当树是一个满二叉树的情况，此时删除任何一个叶子(上图删除8)都会使整棵树不能满足2-3树的定义。这是就得考虑将2-3树的层数减少。办法是将8的双亲和其左子树6合并为一个3结点，再将14与9合并为3结点
...
- 所有的非终端结点中包含下列信息数据
    ...
    - 其中K<sub>i</sub>为关键字，且K<sub>i</sub> < K<sub>i-1</sub>, A<sub>i</sub>为指向子树根结点的指针，且A<sub>i-1</sub>所指向的子树中的所有结点关键字均小于K<sub>i</sub>,A<sub>i</sub>所指向的子树所有关键字均大于K<sub>i</sub>
- 所有的叶子结点都出现在同一层次上，且不携带信息（可以看作 null）。

将2-3-4树插入9个数后的图转成B树，图示如下：

![](../static/images/sjjg_72.png)

B-树的查找和二叉排序树类似，首先从根结点开始，逐层往下顺指针查找结点，然后在结点的关键字中进行查找。

比如我们要查找数字7，7大于5小于8， 若是数字若存在则必在A2所指的子树中，这样找到下一个结点，然后顺序查找到数字7

##### B+树

解决场景：每次经过结点遍历时，都会对结点中的元素进行一次遍历，非常糟糕；有没有可能让遍历时每个元素只访问一次呢?

B+树是应文件系统所需而出的一种B树的变形树。目的是为了解决所有元素遍历等基本问题
...
下图是一颗B+树的示意，灰色关键字即是根结点中的关键字在叶子结点再次列出，并且所有叶子结点都链接在一起

![](../static/images/sjjg_73.png)

- 随机查找：随机查找基本和B-树相同，不同之处是，每次比较关键值相等时，并不会停止，而是会继续向下直到叶子结点
- 最小关键字顺序查找： 从最左侧的叶子结点触发，不经过分支结点，而是沿着指向下一叶子的指针就可遍历所有的关键字

##### 红黑树

红黑树： 是一种二叉排序树(二叉查找树)，但在每个结点上增加一个存储位表示结点的颜色，可以是Red或Black。 通过对任何一条从根到叶子的路径上各个结点着色方式的限制，红黑树确保没有一条路径会比其他路径长出俩倍，因而是接近平衡的。

平衡二叉树(AVL)结构在插入，或删除时，无法避免大量的旋转操作，导致效率有所损耗，但是红黑树，就在这一点有了更好的优化，但是牺牲了平衡性，所以是不是一个完全平衡的二叉搜索树
...
- 红色结点的子结点一定是黑色的
- 红色结点不能相连
- 从任意结点触发，它每一条路径上的黑色结点数目是相同的

> 详细说明和实现：https://blog.csdn.net/patriot_28/article/details/118108539
...
名称 | 收 | 出 | profit | 备注
md | 77600| >85000 | >5000 | 等活动出
称号(春) | 13600000 | 14200000 | 春节活动
